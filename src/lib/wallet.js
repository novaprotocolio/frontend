...
import abi from './abi';
import env from './env';
import { getSelectedAccountWallet, getTransactionReceipt, getContract } from 'sdk-wallet';
...
export const getAllowance = async (tokenAddress, accountAddress) => {
  const contract = getContract(tokenAddress, abi);
  const allowance = await contract.allowance(accountAddress, env.NOVA_PROXY_ADDRESS);
...
export const approve = (tokenAddress, symbol, allowance, action, decimals) => {
  return async (dispatch, getState) => {
    ...
    let spender = get64BytesString(env.NOVA_PROXY_ADDRESS);
...
const watchTransactionStatus = (txID, callback) => {
  const getTransactionStatus = async () => {
    ...
    if (!tx) {
      ...
      // if block is mined, there will be blockNumber != null
      callback(Number(tx.blockNumber) > 0);
    } else {
      alert('success');
    }
  };
