...
import { getSelectedAccount, getSelectedAccountWallet } from 'sdk-wallet';

// request ddex private auth token
export const loginRequest = () => {
  return async (dispatch, getState) => {
    const message = 'NOVADEX-AUTHENTICATION';
    ...
    const novaAuthentication = address + '#' + message + '#' + signature;
    saveLoginData(address, novaAuthentication);
    return dispatch(login(address, novaAuthentication));
  };
};
